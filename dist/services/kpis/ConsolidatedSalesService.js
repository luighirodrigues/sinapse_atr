...
exports.ConsolidatedSalesService = void 0;
const client_1 = require("../../prisma/client");
const resolveClientIdBySlug_1 = require("../tenants/resolveClientIdBySlug");
// Semantica: "orcamentos abertos" sao do proprio dia e zeram diariamente.
class ConsolidatedSalesService {
    async getConsolidatedSalesSummary(params) {
        const clientId = await (0, resolveClientIdBySlug_1.resolveClientIdBySlug)(params.tenantSlug);
        ...
        const [row] = await client_1.prisma.$queryRaw `
      SELECT
    ...
    async getConsolidatedSalesDailySeries(params) {
        const clientId = await (0, resolveClientIdBySlug_1.resolveClientIdBySlug)(params.tenantSlug);
        const startDate = toDate(params.startDate);
        const endDate = toDate(params.endDate);
        const sellerName = normalizeSellerName(params.sellerName);
    ...
    async listConsolidatedSalesSellers(params) {
        const clientId = await (0, resolveClientIdBySlug_1.resolveClientIdBySlug)(params.tenantSlug);
        const rows = await client_1.prisma.$queryRaw `
...
exports.ConsolidatedSalesService = ConsolidatedSalesService;

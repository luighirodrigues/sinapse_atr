...
exports.AnalysisAdminController = void 0;
const SinapseClientRepository_1 = require("../repositories/SinapseClientRepository");
const AnalysisScriptService_1 = require("../services/AnalysisScriptService");
const SessionAnalysisQueueService_1 = require("../services/SessionAnalysisQueueService");
const SessionAnalysisModelService_1 = require("../services/SessionAnalysisModelService");
...
function shortError(err) {
    ...
    const extracted = (typeof data?.error?.message === 'string' && data.error.message) ||
...
class AnalysisAdminController {
    constructor() {
        ...
        this.scriptService = new AnalysisScriptService_1.AnalysisScriptService();
        this.queueService = new SessionAnalysisQueueService_1.SessionAnalysisQueueService();
        this.modelService = new SessionAnalysisModelService_1.SessionAnalysisModelService();
    }
    async createScript(req, res) {
        try {
            ...
            if (!scriptKey)
                return res.status(400).json({ error: 'Missing scriptKey' });
            if (!name)
    ...
    async runSessionAnalyses(req, res) {
        try {
            const clientSlug = String(req.params.clientSlug);
            const client = await this.clientRepo.findBySlug(clientSlug);
            if (!client)
            ...
            if (!scriptKey)
                return res.status(400).json({ error: 'Missing scriptKey' });
            const analysisVersionTag = typeof body.analysisVersionTag === 'string' && body.analysisVersionTag.trim()
            ...
            const criteria = { minMessages, tagFilter: { mode: 'none' }, forceReprocess };
            if (dryRun) {
...
exports.AnalysisAdminController = AnalysisAdminController;

import { ExternalTracking } from './ExternalApiService';
import { SessionType } from '@prisma/client';

export interface NormalizedSession {
  externalTrackingId: number | null; // Null for merged sessions if we lose ID, but we should try to keep one?
  type: SessionType;
  startedAt: Date;
  endedAt: Date | null;
  assignedUser: { name: string; email: string } | null;
  originalTrackings?: number[]; // IDs of trackings merged into this one
}

export class NormalizationService {
  normalizeTrackings(trackings: ExternalTracking[]): { closed: NormalizedSession[]; open: NormalizedSession | null } {
    ...
    const closed = mapped.filter((s) => s.type === SessionType.CLOSED);
    const openReal = mapped.find((s) => s.type === SessionType.OPEN_REAL); // Assuming only one OPEN_REAL can exist?
    const openWeaks = mapped.filter((s) => s.type === SessionType.OPEN_WEAK).sort((a, b) => a.createdAt.getTime() - b.createdAt.getTime());

    // 2. Deduplicate/Merge OPEN_WEAK
    const mergedWeaks: typeof mapped[0][] = [];
    ...
    let finalOpen: NormalizedSession | null = null;
  ...
  assignMessageToSession(messageCreatedAt: Date, sessions: { closed: NormalizedSession[], open: NormalizedSession | null }): string | null {
      // 1. Try CLOSED

...
export type TopSlowestSessionsByTagItem = {
  sessionId: string;
  ticketId: string;
...
type RowWithTags = RowBase & { contactTags: string[] };

export class TopSlowestSessionsByTagService {
  async getTopSlowestSessionsByTag({ clientId, startDate, endDate, tag, limit, includeTags }: GetParams) {
    const normalizedTag = typeof tag === 'string' && tag.trim() ? tag.trim() : undefined;

    if (includeTags) {
      const rows = normalizedTag
        ? await prisma.$queryRaw<RowWithTags[]>`
            WITH Ranked AS (
              SELECT
                s."id" as "sessionId",
